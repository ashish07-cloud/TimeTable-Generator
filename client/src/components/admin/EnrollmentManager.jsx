...
import { 
  Users, Layers, BookCheck, ArrowRight, 
  ChevronLeft, Trash2, Plus, Info, 
  BarChart3, Hash, GraduationCap 
} from 'lucide-react';

const EnrollmentManager = ({ subjects, onNext, onPrev, isLoading }) => {
  const [groups, setGroups] = useState([]);
  const [selectedGroupId, setSelectedGroupId] = useState(null);
  const [activeSubTab, setActiveSubTab] = useState('CORE');

  const [newGroup, setNewGroup] = useState({
    name: '', semester: 1, batchSize: 60, coreIds: [], electiveCounts: {}
  });
  ...
  const handleAddGroup = () => {
    ...
    setNewGroup({ name: '', semester: 1, batchSize: 60, coreIds: [], electiveCounts: {} });
  };
  ...
  const toggleCoreSubject = (subId) => {
    setGroups(prev => prev.map(g => {
      ...
      return {
        ...g,
        coreIds: coreIds.includes(subId) ? coreIds.filter(id => id !== subId) : [...coreIds, subId]
      };
    }));
  };
  ...
  const updateElectiveCount = (subId, count) => {
    setGroups(prev => prev.map(g => {
      ...
      return {
        ...g,
        electiveCounts: { ...g.electiveCounts, [subId]: parseInt(count) || 0 }
      };
    }));
  };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-green-50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/30 p-4 rounded-2xl flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-green-500 text-white rounded-lg"><BarChart3 size={18}/></div>
          <p className="text-sm font-medium dark:text-green-400">
            Total Enrolled: <span className="font-bold">{groups.reduce((acc, g) => acc + g.batchSize, 0)}</span> Students
          </p>
        </div>
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <input 
                  ...
                  onChange={e => setNewGroup({...newGroup, batchSize: e.target.value})}
                  className="w-full px-4 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs outline-none dark:text-white"
                />
              </div>
              <button 
              ...
              {groups.map(g => (
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[10px] bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded font-mono">Sem {g.semester}</span>
                  </div>
        ...
        <div className="lg:col-span-8">
          {selectedGroup ? (
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm flex flex-col h-[600px]">
              ...
              <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Assign mandatory core and elective counts.</p>
                </div>
                  ...
                  {['CORE', 'ELECTIVES'].map(t => (
                    <button 
                      ...
                      className={`px-4 py-1.5 rounded-md text-[10px] font-bold transition-all ${
                        activeSubTab === t ? 'bg-white dark:bg-gray-700 text-green-600 shadow-sm' : 'text-gray-500'
                      }`}
                    >
                      {t}
                    </button>
              ...
              <div className="p-6 flex-1 overflow-y-auto custom-scrollbar">
                {activeSubTab === 'CORE' ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {subjects.filter(s => s.category === 'CORE').map(sub => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-between p-4 rounded-xl border transition-all ${
                            isSelected ? 'bg-green-50 dark:bg-green-900/10 border-green-500/50' : 'border-gray-100 dark:border-gray-800'
                          }`}
                        >
                          <div className="text-left">
                            <p className="text-xs font-bold dark:text-white">{sub.code}</p>
                            <p className="text-[10px] text-gray-500">{sub.name}</p>
                          </div>
                          {isSelected && <BookCheck size={16} className="text-green-600" />}
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <div className="space-y-4">
                    {subjects.filter(s => s.category !== 'CORE').map(sub => (
                      <div key={sub.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800">
                        <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          <label className="text-[10px] font-bold text-gray-400">ENROLLED:</label>
                          <input 
                            type="number" 
            ...
            <div className="h-[600px] flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900/50 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl text-gray-400">
              ...
              <p className="font-medium text-sm">Create and select a batch to manage enrollment.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mt-10">
        <button onClick={onPrev} className="px-6 py-3 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 font-bold hover:bg-gray-100 transition-all">
          Back
        </button>
        <button 
